...
macro_rules! define_id_type {
    ($name:ident, $backing:ty, $prefix:literal) => {
        ...
        impl_sql_from_str_display!($name);
    };
}

define_id_type!(IndividualID, Curve25519PubKey, "05");
define_id_type!(GroupID, ED25519PubKey, "03");
define_id_type!(Blind15ID, ED25519PubKey, "15");
define_id_type!(Blind25ID, ED25519PubKey, "25");

macro_rules! define_id_combinations {
    ($name:ident $(,$id_type:ident)+) => {
        paste! {
            #[derive(Clone, Debug, SerializeDisplay, DeserializeFromStr, Eq, PartialEq, Hash, Ord, PartialOrd)]
            pub enum $name {
                $(
                    $id_type([<$id_type ID>]),
                )+
...
define_id_combinations!(SessionID, Individual, Group, Blind15, Blind25);
define_id_combinations!(IndividualOrBlindedID, Individual, Blind15, Blind25);
define_id_combinations!(IndividualOrGroupID, Individual, Group);
define_id_combinations!(BlindID, Blind15, Blind25);
...
define_session_id_conversion!(IndividualOrBlindedID, Individual, Blind15, Blind25);

pub static MENTION_ID_PATTERN: LazyLock<Regex> =
    LazyLock::new(|| Regex::new("@(05|03|15|25)([0-9a-fA-F]{64})(\\s|$)").unwrap());

pub fn extract_mention_id(capture: Captures) -> &str {
    &capture.get(0).unwrap().as_str()[1..67]
}
