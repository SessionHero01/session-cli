...
impl<T> CWrapper<T> {
    ...
    pub fn new_with_c_free(ptr: *mut T) -> Option<Self> {
        if ptr.is_null() {
            None
        } else {
            Some(Self {
                ptr,
                destroy: Some(Self::c_free_destroy),
            })
        }
    }

    unsafe extern "C" fn c_free_destroy(ptr: *mut T) {
        unsafe { crate::bindings::free(ptr as *mut c_void) }
    }
