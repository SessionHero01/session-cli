use super::{
    ConfigPush, GroupAuthData, GroupInfoConfig, GroupMemberConfig, wrapper::ConfigWrapper,
};
use crate::bindings;
...
pub struct GroupKeys {
    ...
    group_info_config: ConfigWrapper<GroupInfoConfig>,
    group_members_config: ConfigWrapper<GroupMemberConfig>,
}
...
impl GroupKeys {
    pub fn new(
        ...
        let rc = unsafe {
            bindings::groups_keys_init(
                ...
                group_info_config.borrow_mut().as_mut() as *mut _,
                group_members_config.borrow_mut().as_mut() as *mut _,
                if dump.is_empty() {
                    null()
                } else {
                    dump.as_ptr()
    ...
    pub fn encrypt_message(&self, plaintext: &[u8]) -> Option<impl AsRef<[u8]> + use<>> {
    ...
    pub fn decrypt_message(
        &self,
        cipher_text: &[u8],
    ) -> Option<(SessionID, impl AsRef<[u8]> + use<>)> {
    ...
    pub fn len(&self) -> usize {
        unsafe { bindings::groups_keys_size(self.wrapper.as_ptr()) }
    }
}

impl super::NamedConfig for GroupKeys {
    const CONFIG_TYPE_NAME: &'static str = "GroupKeys";
    const NAMESPACE: MessageNamespace = MessageNamespace::GroupKeysConfig;
}
...
impl super::Config for GroupKeys {
            ...
            .map(|msg| {
                let success = unsafe {
                    bindings::groups_keys_load_message(
                        ...
                        msg.data.as_ptr(),
                        msg.data.len(),
                        msg.sent.as_millis() as i64,
                        self.group_info_config.borrow_mut().as_mut() as *mut _,
                        self.group_members_config.borrow_mut().as_mut() as *mut _,
                    )
                };
                ...
                if success {
                    ...
                    bail!("Failed to load key message")
                }
            })
            .collect()
    }
    ...
    fn active_hashes(&self) -> Vec<String> {
        let hashes = unsafe { bindings::groups_keys_active_hashes(self.wrapper.as_ptr()) };
        CWrapper::new(hashes)
            .iter()
            .flat_map(|c| {
                super::config_string_list_as_slice(c.as_ref())
                    .into_iter()
    ...
    fn push(&mut self) -> anyhow::Result<Option<ConfigPush>> {
        let Some(data) = self.pending_config() else {
            return Ok(None);
        };

        Ok(Some(ConfigPush {
            data: vec![data.as_slice().to_vec()],
            obsolete_hashes: Default::default(),
            seq: 0,
        }))
    }
