...
impl OnionRequestBuilder {
    ...
    pub fn set_snode_destination(
        ...
        unsafe {
            ...
            bindings::onion_request_builder_set_destination_pubkey(
                self.0.as_mut_ptr(),
    ...
    pub fn set_server_destination(
        ...
        unsafe {
            bindings::onion_request_builder_set_server_destination(
                ...
                url.host_str()
                    .ok_or("Failed to get host")?
                    .to_cstr()
                    .as_ref()
                    .as_ptr(),
                url.path_and_query().to_cstr().as_ref().as_ptr(),
                method.as_str().to_cstr().as_ref().as_ptr(),
                url.port_or_known_default().ok_or("Failed to get port")?,
